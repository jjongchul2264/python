window.onload = function () {
    ...
    if (employeeCode && year) {
        fetch(`/getContractData2?code=${employeeCode}&year=${year}`)
            ...
            .then(data => {
                ...
                document.getElementById("val01").innerText = data.val01;
                document.getElementById("val02").innerText = data.val02;
                ...
                //연구활동비
                document.getElementById("amt4").innerText = data.amt5;
                //벽오지 만근수당 / 직무수당
                document.getElementById("amt5").innerText = data.amt6;
                //생산수당 / 육아수당
                document.getElementById("amt6").innerText = data.amt8;
                //합계
                document.getElementById("amt7").innerText = data.amt7;
                //상여금
                document.getElementById("amt8").innerText = data.amt7;
                //기타수당 포함해야할거 같아보임..
                //document.getElementById('amt9').innerText = data.amt8;
            })
    ...
    fetch(`/get_signature?emp_code=${empCode}&year=${contractyear}`)
        ...
        .then(data => {
            if (data.signature) {
                ...
                signatureImage.src = `data:image/png;base64,${data.signature}`;
